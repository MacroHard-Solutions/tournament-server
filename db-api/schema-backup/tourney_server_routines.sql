...
BEGIN
	SET @aID = agentID;
    SET @moveStmt = 'INSERT INTO AGENT_ARCHIVE (AGENT_ID, AGENT_NAME, USER_ID)
    ...
    SET @moveStmt = 'INSERT INTO RANKING_ARCHIVE (MATCH_LOG_ID, AGENT_ID, RANKING)
			SELECT MATCH_LOG_ID, AGENT_ID, RANKING
			FROM RANKING
            WHERE AGENT_ID = ?';
...
BEGIN
	SET @aID = agentID;
    SET @selectStmt = 'SELECT A.AGENT_ID, AGENT_NAME, U.USER_ID, U.USERNAME, AD.ADDRESS_IP, AD.ADDRESS_PORT, A.AGENT_STATUS
			FROM AGENT AS A
...
BEGIN
	SET @uID = userID;
	SET @getAgents = 'SELECT DISTINCT A.AGENT_ID, A.AGENT_NAME, AD.ADDRESS_IP, AD.ADDRESS_PORT, GAME_NAME, USERNAME, AGENT_ELO,  AGENT_STATUS, T.TOURNAMENT_ID, T.TOURNAMENT_NAME
		FROM USER AS U 
...
-- Dump completed on 2022-07-25 22:44:20
